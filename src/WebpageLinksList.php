...
{
	public function __construct( string $url, int $limit = -1, bool $html_only = true )
	{
		$this->tried_list = [];
		$this->list = [];
		$this->data = [];
	...
	private function tryURL( string $url, URI $host ) : void
	{
		...
		try
		{
			...
			if ( $this->html_only && $this->testResponseNotHTML( $response ) )
			{
				//var_dump( "URL {$url} is not HTML." );
				return;
			}
			$this->list[] = $url;
